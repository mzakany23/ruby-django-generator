...
describe HtmlTagParser do 
	before do 
		...
		@new_template_index = '/Users/mzakany/Desktop/new_template/index.html'
		@parser = HtmlTagParser.new(@new_template_index,'/Users/mzakany/Desktop/file.html')
		@new_parser = HtmlTagParser.new('../../files/settings.py','/Users/mzakany/Desktop')
	end

	it 'will return every instance of the tag match' do 
		tags_to_look_for = {
			img: ' <img',
			css: '<link href',
			css2: '<link rel',
		...
		# @parser.return_formatted_string(tags_to_look_for).to_html
		@parser.return_formatted_string(tags_to_look_for).to_html
	end

	# it 'will generate a file with django tags to desktop' do 
