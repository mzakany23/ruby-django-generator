...
class HtmlTagParser 
	...
	def return_regex(short_name,line)
		...
		if short_name == :img
			...
			new_formatted_line = "<img src="  "{% static '#{match[:middle]}' %} " "class='#{match[:class]}' + alt=''"
		elsif short_name == :css
			match = /\<\w{0,5}.\w{0,4}.\w{0,10}.\w{0,5}\=(?<css>[^.]{0,20}\.[^\s]{0,20})/.match(whole_line_de_stringed)
			new_formatted_line = "<link rel='stylesheet' " "{% static '#{match[:css]}' %}" "type='text/css'>"
		elsif short_name == :js
			match = /\<\w{0,5}.\w{0,4}.\w{0,10}.\w{0,5}\=(?<css>[^.]{0,20}\.[^\s]{0,20})\>/.match(whole_line_de_stringed)
			new_formatted_line = "<script src=" "{% static '#{match[:css]}' %}" "> <script>"	
		end
